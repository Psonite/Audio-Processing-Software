...
def  main():
  ...
  installer = "\installer\FlangerPlugin Installer.exe"
   
  if demo:
    installer = "\installer\FlangerPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FlangerPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "FlangerPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FlangerPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
