...
def  main():
  ...
  installer = "\installer\CompressorPlugin Installer.exe"
   
  if demo:
    installer = "\installer\CompressorPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\CompressorPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "CompressorPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "CompressorPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
