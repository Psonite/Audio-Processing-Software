...
def  main():
  ...
  installer = "\installer\DelayPlugin Installer.exe"
   
  if demo:
    installer = "\installer\DelayPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DelayPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "DelayPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DelayPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
