...
if %1 == 1 (echo Making DelayPlugin Windows DEMO VERSION distribution ...) else (echo Making DelayPlugin Windows FULL VERSION distribution ...)
...
REM - msbuild DelayPlugin-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild DelayPlugin.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild DelayPlugin.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\DelayPlugin.aaxplugin\Contents\Win32\DelayPlugin.aaxplugin --out .\build-win\aax\bin\DelayPlugin.aaxplugin\Contents\Win32\DelayPlugin.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\DelayPlugin.aaxplugin\Contents\x64\DelayPlugin.aaxplugin --out .\build-win\aax\bin\DelayPlugin.aaxplugin\Contents\x64\DelayPlugin.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\DelayPlugin.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\DelayPlugin.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "DelayPlugin Installer" ".\installer\DelayPlugin Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\DelayPlugin Installer.exe" ".\installer\DelayPlugin Demo Installer.exe"
REM -del ".\installer\DelayPlugin Installer.exe"
REM -)
