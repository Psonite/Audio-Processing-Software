...
def  main():
  ...
  installer = "\installer\DistortionPlugin Installer.exe"
   
  if demo:
    installer = "\installer\DistortionPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DistortionPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "DistortionPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DistortionPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
